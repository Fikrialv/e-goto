<x-layouts.app :title="$awal ? 'Lengkapi profil' : 'Profil'">
    <div class="mx-auto w-full max-w-2xl px-4 py-12 sm:px-6 lg:py-16">
        @if ($awal)
            <p class="text-xs font-semibold tracking-widest text-amber-700 uppercase">Langkah terakhir</p>
        @endif
        ...
        <form method="POST" action="{{ route('profile.update') }}" class="mt-8 grid gap-5 rounded-3xl border border-mist-200 bg-white/70 p-6 sm:p-8">
            ...
            <div class="mt-2 flex flex-col gap-3 sm:flex-row sm:items-center">
                ...
                @if ($awal)
                    <a href="{{ route('profile.skip') }}" class="text-center text-sm text-teal-600 underline underline-offset-4 sm:text-left">
                        Nanti saja
                    </a>
                @endif
            </div>
