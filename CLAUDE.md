...
Lima aturan warna & aset yang ditetapkan 2026-08-28 (yang terakhir 2026-08-29) dan berlaku permanen:

- **Latar halaman customer wajib putih murni (#FFFFFF).** Dilarang latar krem/off-white hangat — `bg-amber-50`, `bg-orange-50`, `bg-stone-50`, `bg-yellow-50`, `bg-neutral-50`, atau hex apa pun yang condong kuning/coklat muda. Sebabnya bukan selera: token-token itu tidak didefinisikan di `@theme` project, jadi Tailwind memakai palet bawaannya yang hangat dan hasilnya terbaca krem/kotor di samping teal. Pita `mist-100` tetap boleh untuk selang-seling seksi karena huenya dingin. `amber-100` boleh untuk bidang kecil (badge, lingkaran ikon); untuk panel besar pakai `bg-white` + border amber.
- **Tidak ada warna dominan di luar palet E-GOTO.** Teal (brand), amber (CTA/urgensi), teal gelap (teks). Dilarang violet, purple, indigo, hijau lumut, biru korporat sebagai elemen dominan. Moodboard dan referensi visual apa pun dipakai **hanya** untuk pola layout — susunan elemen, jenis komponen, treatment visual — warnanya selalu diganti ke token E-GOTO. Sebelum lapor selesai, jalankan `grep -rnE "violet|purple|indigo|(bg|text|border)-(amber|orange|stone|yellow|neutral)-50([^0-9]|$)" resources/views/ resources/css/` dan pastikan hasilnya nol.
- **Berkas logo dipilih sesuai konteks, jangan satu untuk semua.** `Logo1.svg` wordmark horizontal — header situs, header form masuk, footer, header e-tiket, `brandLogo` Filament. Header dan footer memakai berkasnya, bukan teks `E·GOTO`. `logo2.svg` bentuk saja tanpa teks — loading screen, overlay submit, favicon. Tidak ada berkas ketiga; jangan mengarang turunan tanpa diminta.
- **Dilarang library carousel/slider/animasi pihak ketiga** (Swiper, Slick, Glide, GSAP, framer-motion, Lottie). Alpine `x-data` + `x-transition` + CSS sudah cukup untuk semua gerakan di project ini, dan tiap library tambahan melawan aturan performa §9. Efek glass (`backdrop-blur`) dibatasi ke panel halaman masuk saja.
- **Ikon tidak diwarnai.** Ikon mewarisi warna teks di sekitarnya, dan warna teks itu selalu dari tangga teal (`teal-900`/`teal-700`/`teal-500`). Amber dilarang untuk ikon — termasuk bintang rating — karena amber adalah penanda urgensi dan tombol aksi; begitu ikon ikut kuning, penandanya kehilangan arti. Pengecualian tunggal: ikon di dalam tombol amber mewarisi warna teks tombolnya. Verifikasi: `grep -rn "lucide.*text-amber" resources/views/` harus nol.
...
## 18. Pemicu cepat: "nyalakan rtk dan caveman"

Kalau user menulis kalimat yang isinya minta menyalakan rtk dan/atau caveman — apa pun susunan katanya ("nyalakan rtk dan caveman", "rtk caveman on", "hidupkan caveman") — perlakukan sama dengan menjalankan `/nyalakan` (lihat `.claude/commands/nyalakan.md`): verifikasi RTK lalu panggil skill `caveman:caveman`, dan lapor status dua baris. Jangan tanya konfirmasi dulu, langsung eksekusi.

Catatan: hook `rtk hook claude` sudah terpasang di `PreToolUse` pada `~/.claude/settings.json`, jadi RTK sebenarnya aktif otomatis tiap sesi tanpa diminta. Yang benar-benar perlu dinyalakan manual tiap sesi cuma caveman mode.
