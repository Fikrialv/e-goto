...
- [ ] Widget chat pihak ketiga (Tawk.to **atau** Crisp — pilih satu saat eksekusi) di layout utama customer
- [ ] Tombol "Lanjut ke WhatsApp" di dalam widget
- [ ] Batas keras ditegakkan: widget hanya chat umum/CS — approve/reject pembayaran & penerbitan tiket tetap lewat Filament D5
- [ ] Nama penyedia widget dicantumkan di `/kebijakan-privasi` (pihak ketiga penerima data)
- [ ] Web Push opt-in: tombol izin di Booking Saya/profil (jangan prompt otomatis)
- [ ] Push terkirim untuk pembayaran `verified` + pengingat H-1, tanpa NIK/paspor, tanpa worker daemon
